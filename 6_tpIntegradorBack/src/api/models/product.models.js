...
const selectAllProducts = () => {
     //Optimizacion 3: Extraemos la sentencia de una variables y la optimizamos
        ...
        const sql = "SELECT id, name, price, image, category, activo FROM productos";
...
const updateProduct = (name, image, price, category, activo, id) => {
    const sql = "UPDATE productos SET name = ?, image = ?, price = ?, category = ?, activo = ? WHERE id = ?";
        
    return connection.query(sql, [name, image, price, category, activo, id]);
}
